...
export const useSessions = (limit = 50) =>
  useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<SessionSummary[]>(`/api/v1/sessions?limit=${limit}`)
      return data
    },
    staleTime: 30_000,
  })
...
export const useSession = (id: string, enabled = true) =>
  useQuery({
    queryKey: ['session', id],
    queryFn: async () => {
      const { data } = await api.get<Session>(`/api/v1/sessions/${id}`)
      return data
    },
    enabled,
  })
...
export const useCreateSession = () => {
  ...
  const m = useMutation<SessionSummary, unknown, CreateSessionBody>({
    mutationFn: async (body) => (await api.post('/api/v1/sessions', body)).data,
...
export const useUpdateSession = () => {
  ...
  const m = useMutation<void, unknown, { id: string; changes: UpdateSessionBody }>({
    mutationFn: async ({ id, changes }) => {
      await api.patch(`/api/v1/sessions/${id}`, changes)
    },
    onSuccess: (_d, { id }) => {
