  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ 
                ...
                <Box
                  sx={{
                    ...
                    background: m.from === 'user' 
                      ? 'linear-gradient(135deg, #2979FF 0%, #2962FF 100%)'
                    ...
                    boxShadow: m.from === 'user'
                      ? '0 2px 12px rgba(41, 121, 255, 0.3)'
                      : theme.palette.mode === 'dark'
        ...
        {/* Quick replies */}
        {showQuickReplies && msgs.length === 1 && (
          <Fade in timeout={800}>
            <Box sx={{ mt: 1, mb: 2 }}>
              <Typography variant="caption" sx={{ color: theme.palette.text.secondary, mb: 1, display: 'block', pl: 1 }}>
                Suggested topics:
              </Typography>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {QUICK_REPLIES.map((reply: string) => (
                  <Chip
                    key={reply}
                    label={reply}
